...
from robots.models import Robot, RobotModel, RobotVersion  # isort:skip
...
class RobotWriteOnlySerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not RobotVersion.objects.filter(
            model=model, version=data["version"]
        ).exists():
            raise serializers.ValidationError(
    ...
    def create(self, validated_data):
        ...
        version = RobotVersion.objects.get(
            model=model, version=validated_data["version"])
